...
<div class="flex w-screen z-10 h-screen dark:bg-indigo-400 bg-indigo-200">
	<div class={`${style.default} ${$sidebarOpen ? style.open : style.close}`}>
		...
		<section
			class="flex-1 overflow-y-auto relative dark:text-dark-text section main-section dark:shadow-none dark:bg-indigo-400 md:rounded-xl text-light-text"
		>
			<slot />
		</section>
	</div>
